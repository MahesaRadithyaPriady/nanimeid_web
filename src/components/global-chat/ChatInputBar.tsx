  ...
  return (
    <div className="bg-bg-elevated border-t border-border/40 p-2 sm:p-3 pb-safe z-10 shrink-0 relative">
      <div className="w-full max-w-4xl mx-auto flex flex-col gap-2 relative">
        {/* Sticker Picker */}
        {showStickerPicker && (
          <div className="absolute bottom-full left-0 mb-2 w-72 max-h-64 bg-bg-elevated/95 backdrop-blur-xl border border-border/50 dark:border-white/10 rounded-xl shadow-2xl overflow-y-auto scrollbar-pink z-50 p-2 grid grid-cols-4 gap-2">
            {stickerLoading ? (
              ...
              stickers.map((stk, idx) => (
                <button 
                  ...
                  className="aspect-square bg-black/5 dark:bg-white/5 rounded-lg hover:bg-black/10 dark:hover:bg-white/10 hover:scale-105 active:scale-95 transition-all flex items-center justify-center p-1"
        ...
        {/* Reply Preview */}
        {replyingTo && (
          <div className="flex items-center justify-between bg-black/5 dark:bg-black/20 rounded-lg p-2 px-3 mx-1 mb-1 border-l-4 border-primary/50 relative">
            <div className="flex flex-col flex-1 truncate pr-4">
              <span className="text-xs text-pink-600 dark:text-primary font-semibold mb-0.5">
            ...
            <button onClick={onCancelReply} className="p-1 hover:bg-black/5 dark:hover:bg-white/10 rounded-full text-text-secondary hover:text-text-primary shrink-0 transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
        )}
        ...
        {/* Image Preview */}
        {image && (
          <div className="mx-1 mb-1 relative w-20 h-20 rounded-md overflow-hidden border border-border/50 dark:border-white/20">
        ...
        {/* Input Area */}
        <div className="flex items-end gap-2 bg-bg-base dark:bg-bg-base/50 p-1.5 rounded-2xl border border-border/50 focus-within:border-primary/50 transition-colors">
          
          {/* Sticker Button */}
          <button 
            onClick={() => setShowStickerPicker(!showStickerPicker)}
            className={`p-2.5 rounded-xl transition-colors shrink-0 ${showStickerPicker ? 'text-primary bg-primary/10' : 'text-text-secondary hover:text-primary hover:bg-black/5 dark:hover:bg-white/5'}`}
            title="Kirim Stiker"
          >
          ...
          <button 
            ...
            className="p-2.5 text-text-secondary hover:text-primary rounded-xl hover:bg-black/5 dark:hover:bg-white/5 transition-colors shrink-0"
            title="Kirim Gambar"
          >
          ...
          <button 
            ...
            className={`p-2.5 rounded-xl transition-all shrink-0 ${
              text.trim() || image 
                ...
                : 'bg-black/5 dark:bg-white/5 text-text-secondary/30 dark:text-text-secondary/50 cursor-not-allowed'
            }`}
          >
            {isSending ? (
